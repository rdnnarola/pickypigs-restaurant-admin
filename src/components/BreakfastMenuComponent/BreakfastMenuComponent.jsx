...
import { getSelectedMenuDishData, showAddUpdateMenuModal } from "../../redux/actions/menuAction";
...
const BreakfastMenuComponent = () => {
    ...
    return (
        <>
            ...
            {availablityLoading&&availablityLoading?
                <CustomLoadingComp/>
                :
                null
            }
            <div className="row">
                <div className="col-sm-12">
                    <div className="page-heading brandon-Medium">
                        <h4 className="text-capitalize">{selectedMenuDiscInfo_data&&selectedMenuDiscInfo_data.name}</h4>
                    </div>
                </div>
            </div>
            <div className="row">
                <div className="col-sm-12 mt-4 mb-4">
                    <div className="d-flex justify-content-between align-items-center flex-wrap">
                        <div className="custom-control custom-checkbox pink-checkbox">
                            <input type="checkbox" className="custom-control-input" id="customCheck199" defaultChecked={showDesc} onClick={(e)=>{setShowDesc(e.target.checked)}} />
                            <label className="custom-control-label" htmlFor="customCheck199">Show description</label>
                        </div>
                        <button onClick={() => {dispatch(showAddUpdateMenuModal(true));}} className="btn pinkline-btn text-uppercase rounded-pill f-15"><span className="edit-icon">EDIT MENU</span></button>
                    </div>
                </div>
                <AddEditMenuModalComp  show={addMenuModalShow} onHide={() => dispatch(showAddUpdateMenuModal(false))} menuid={MenuInfoId?MenuInfoId:""} showDeleted={false}/>
            </div>
           {selectedMenuDiscInfo_data&&selectedMenuDiscInfo_data.categories&&selectedMenuDiscInfo_data.categories.length>0?
                <React.Fragment>
                    {selectedMenuDiscInfo_data&&selectedMenuDiscInfo_data.categories&&selectedMenuDiscInfo_data.categories.map((data,index)=>{
                        return(
                                    ...
                                    <div className="col-sm-12">
                                        <div className="table-responsive my_custom_table table-accordion mb-4">
                                            <table className="table table-striped table-main">
                                                <thead >
                                                    <tr>
                                                        <th className="brandon-Bold text-uppercase" scope="col">{data.name?data.name:'unknown'}</th>
                                                        <th className="brandon-Bold text-center" scope="col">ALLERGIES</th>
                                                        <th className="brandon-Bold text-right" scope="col">GROSS PROFIT(%)</th>
                                                ...
                                                <tbody>
                                                    {data&&data.subcategories&&data.subcategories.length>0?
                                                        <React.Fragment>
                                                            <React.Fragment>
                                                                {data&&data.subcategories&&data.subcategories.filter(mydata=>mydata._id == null).map((data2,index2) => {
                                                                    return (
                                                                                            ...
                                                                                            <React.Fragment key={index3}>
                                                                                                <tr >   
                                                                                                    <td className="text-capitalize">
                                                                                                        ...
                                                                                                        {showDesc && (
                                                                                                            <p className="dish-description gray-txt">{data3.description?data3.description:"Dish Description Not Available"}</p>
                                                                                                        )}
                                                                                                    </td>
                                                                                                    ...
                                                                                                    <td className="text-right">{data3.grossProfit?`${data3.grossProfit}%`:"-"}</td>
                                                                                                    <td className="text-right">{data3.priceUnit?data3.priceUnit:"$"}{data3.price?data3.price.toFixed(2):"-"}</td>
                                                                                                    <td className="text-center">
                                                                                                        <div className="custom-control custom-checkbox pink-checkbox">
                                                                                                            <input type="checkbox" className="custom-control-input" id={data3._id} onClick={(e)=>{dispatch(updateSelectedDishAvailablity(data3._id,{available:e.target.checked}))}} defaultChecked={data3.available}/>
                                                                                                            <label className="custom-control-label" htmlFor={data3._id}></label>
                                                                                                        </div>
                                                                                                    </td>
                                                                                                </tr>
                                                                                                {/* {showDesc?
                                                                                        ...
                                                                                        <td colSpan="5" className="border-top-0">
                                                                                            <div className="sub-category-data">
                                                                                                <table className="w-100 table table-striped table-main">
                                                                                                    <thead>
                                                                                                        <tr className="accordion-toggle collapsed mt-2 ml-5 " id={`accordion${index2+1}`} data-toggle="collapse" data-parent={`#accordion${index2+1}`} href={`#collapse${index2+1}`}>
                                                                                                            <th scope="col" className="font-weight-bold text-uppercase">{data2.name?data2.name:'Unknown'}</th>
                                                                                                            ...
                                                                                                            return (
                                                                                                                <React.Fragment key={index3}>
                                                                                                                    <tr id={`collapse${index2+1}`} className="collapse in p-3 hide-table-padding">
                                                                                                                        <td className="text-capitalize">{data3.name?data3.name:''}
                                                                                                                        {showDesc && (
                                                                                                                            <p className="dish-description gray-txt">{data3.description?data3.description:"Dish Description Not Available"}</p>
                                                                                                                        )}
                                                                                                                        </td>
                ...
                <React.Fragment>
                    <div className="row">
                        <div className="col-sm-12" style={{borderRadius:20,boxShadow:"0px 1px 20px 0px #00000018"}}>
