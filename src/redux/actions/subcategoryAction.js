  ...
  return async (dispatch) => {
    try {
      ...
      let response = await Axios.get(
        `/restaurant_admin/subcategory/${subCategoryId}`
      );
      dispatch({
        type: "GET_SELECTEDSUBCATEGORY_SUCCESS",
...
export const getSubCategoryListOfSelectedCategory = (selectedId) => {
  return async (dispatch) => {
    try {
      ...
      dispatch({
        type: "GET_SELECTEDSUBCATEGORYLIST_SUCCESS",
        payload: response.data,
      });
    } catch (error) {
...
export const hideSelectedSubCategoryData = (selectedId, data) => {
  return async (dispatch) => {
    try {
      ...
      dispatch(
        setAlert(
          `Category ${data.isActive ? "UnHide" : "Hide"} Successfuly`,
          "success"
        )
      );
    } catch (error) {
...
export const duplicateSelectedSubCategoryData = (selectedId, data) => {
  return async (dispatch) => {
    try {
      ...
      let response = await Axios.put(dataURL, JSON.stringify(data), config);
      dispatch({
        type: "DUPLICATE_SUBCATEGORY_SUCCESS",
        payload: response.data,
      });
      dispatch(getAllSubCategoryData());
