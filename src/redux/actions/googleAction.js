...
export const getLocationGeometryData = (position,coordinate,addressName) =>  {
    ...
    return async(dispatch)=>{
      try{
          ...
          let pincode=position.filter(myallergy => myallergy.types.indexOf("postal_code") !== -1)
          dispatch(getPostalCodeData(pincode[0]?pincode[0].long_name:''))
          let street=position.filter(mystreet => mystreet.types.indexOf("route") !== -1)
          dispatch(getStreetNameData(street[0]?street[0].long_name:''))
          let location=position.filter(mystreet => mystreet.types.indexOf("locality") !== -1)
